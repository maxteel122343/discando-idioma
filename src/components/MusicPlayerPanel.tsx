...
export const MusicPlayerPanel: React.FC<MusicPlayerPanelProps> = ({
  ...
  return (
    <div className="bg-white/85 dark:bg-slate-900/85 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800/80 rounded-3xl p-4 sm:p-5 shadow-2xl space-y-4 max-w-xl mx-auto transition-all">
      ...
      {/* Lyrics Line Selector */}
      <div className="space-y-2">
        <div className="flex items-center justify-between text-[11px] font-mono font-black text-slate-400 uppercase tracking-wider px-1">
          <span>Letra da Música (Linhas de Discagem)</span>
          <span>Selecione para Praticar</span>
        </div>

        <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
          {sentences.map((sentence, idx) => {
            const isActive = activeSentenceIndex === idx;
            const isCompleted = completedSentenceIndices.includes(idx);
              ...
              >
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    playTick();
                    onSpeakSentence(joinSentence(sentence.characters));
                  }}
                  className="p-2 rounded-xl bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-800 hover:bg-slate-100 active:scale-90 transition-all"
